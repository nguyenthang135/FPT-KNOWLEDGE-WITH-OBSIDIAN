...
class _FlmLoginScreenState extends State<FlmLoginScreen> {
  ...
  Future<void> _initialize() async {
    try {
      final keepSignedIn = await _settings.getKeepMeSignedIn();
      ...
      if (!keepSignedIn) {
        await _session.controller.clearCookies();

        await _session.controller.clearCache();
      }
  ...
  Future<void> _changeKeepSignedIn(bool value) async {
    setState(() {
      _keepMeSignedIn = value;
    });

    await _settings.setKeepMeSignedIn(value);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Login to FPT FLM')),
      body: Column(
        children: [
          Container(
              ...
              children: [
                Checkbox(
                  ...
                  onChanged: _initializing
                      ? null
                      : (value) {
                          _changeKeepSignedIn(value ?? false);
                        },
                ),
