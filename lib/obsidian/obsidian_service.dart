...
typedef ObsidianProgressCallback = void Function(
  int current,
  int total,
  String message,
);
...
class _SyncState {
  ...
  factory _SyncState.fromJson(
    ...
    final rawCodes =
        json['specializationSubjectCodes'];
...
class ObsidianService {
  final VaultPicker _picker =
      const VaultPicker();

  final FlmSyllabusService
      _syllabusService =
      FlmSyllabusService();
  ...
  Future<ObsidianExportResult?>
      exportCurriculum({
    required String curriculumCode,
    required List<CurriculumSubject>
        curriculumSubjects,
    required SpecializationCombo?
        specialization,
    ...
    if (!await vaultDirectory.exists()) {
      throw Exception(
        'The saved Obsidian vault no longer exists.',
      );
    }
    ...
    final looksLikeObsidianVault =
        await obsidianConfigDirectory
            .exists();

    final safeCurriculumCode =
        _safeName(
      curriculumCode,
    );
    ...
    final archiveDirectory =
      ...
      _join(
        curriculumRoot.path,
        'Archived Specialization',
      ),
    );

    await curriculumRoot.create(
      recursive: true,
    );

    await semestersDirectory.create(
      recursive: true,
    );

    await subjectsDirectory.create(
      recursive: true,
    );

    if (specialization != null) {
      await specializationDirectory
          .create(
        recursive: true,
      );
    }
    ...
    final currentSpecializationKeys =
        currentSpecializationCodes
            .map(_subjectKey)
            .toSet();

    final currentSpecializationName =
    ...
    final manifestFile =
      ...
      _join(
        curriculumRoot.path,
        '.fpt_knowledge_sync.json',
      ),
    );

    final previousState =
        await _readSyncState(
      manifestFile,
    );

    String? previousSpecializationName =
        previousState
            ?.specializationName;

    Set<String>
        previousSpecializationKeys;
    ...
    if (previousState != null) {
      ...
      previousSpecializationKeys =
          await _inferOldSpecializationSubjects(
        subjectsDirectory:
            subjectsDirectory,
        baseSubjectKeys:
            baseSubjectKeys,
      ...
      previousSpecializationName ??=
          await _inferOldSpecializationName(
        specializationDirectory:
            specializationDirectory,
    ...
    final staleSpecializationKeys =
        previousSpecializationKeys
            .difference(
              currentSpecializationKeys,
            )
            .where(
              (key) =>
                  !baseSubjectKeys
    ...
    if (specialization != null) {
      final specializationFileName =
          _safeName(
        specialization.name.isEmpty
            ? 'Specialization'
            : specialization.name,
      );
      ...
      await _writeGeneratedFile(
        File(
          _join(
            specializationDirectory.path,
            '$specializationFileName.md',
          ),
        ),
      ...
      final personalSpecializationNotes =
          File(
        _join(
          specializationDirectory.path,
      ...
      if (!await personalSpecializationNotes
          .exists()) {
        await personalSpecializationNotes
            .writeAsString(
          '''
...
''',
    ...
    for (var index = 0;
      ...
      final isCurrentSpecializationSubject =
          currentSpecializationKeys
              .contains(
        subjectKey,
      );

      final safeSubjectCode =
          _safeName(
        subject.code,
      );

      final subjectDirectory =
          Directory(
        _join(
          subjectsDirectory.path,
          safeSubjectCode,
        ),
      );
  ...
  Future<_SyncState?> _readSyncState(
    File file,
  ) async {
    try {
      if (!await file.exists()) {
  ...
  Future<Set<String>>
      _inferOldSpecializationSubjects({
  ...
  Future<String?>
      _inferOldSpecializationName({
    required Directory
        specializationDirectory,
    required String?
        currentSpecializationName,
  }) async {
    if (!await specializationDirectory
        .exists()) {
      return null;
    }
    ...
    await for (final entity
        in specializationDirectory.list(
      followLinks: false,
    )) {
      if (entity is! File) {
        continue;
  ...
  Future<void>
    ...
    final containsUserData =
        await _containsUserOwnedData(
      source,
    );
    ...
    await archiveSpecialization.create(
      recursive: true,
    );

    final desiredDestination =
        _join(
  ...
  Future<void>
    ...
    await specializationDirectory.create(
      recursive: true,
    );
  ...
  Future<void>
    ...
    if (await activeSubjectDirectory
        .exists()) {
      return;
    }

    final archivedDirectory =
        Directory(
      _join(
        _join(
          archiveDirectory.path,
  ...
  Future<void>
      _removeOldGeneratedSpecializationPages({
    required Directory
        specializationDirectory,
    required String?
        currentSpecializationName,
    ...
    final currentFilename =
        currentSpecializationName != null
            ? '${_safeName(currentSpecializationName)}.md'
                .toLowerCase()
            : null;
  ...
  String _buildCurriculumIndex({
    ...
    buffer.writeln(
      '> Curriculum information synchronized from FPT FLM.',
    );
  ...
  String _buildOverviewMarkdown({
    ...
    if (syllabus.timeAllocation
      ...
      buffer.writeln(
        syllabus.timeAllocation.trim(),
      );
      buffer.writeln();
    }

    if (syllabus.teachingMethod
        .trim()
        .isNotEmpty) {
      buffer.writeln(
        '## Teaching Method',
  ...
  String _buildAssessmentMarkdown({
    ...
    if (syllabus == null ||
        syllabus.assessments.isEmpty) {
      buffer.writeln(
        'No assessment information was retrieved from FLM.',
      );
  ...
  String _buildMaterialsMarkdown({
    ...
    buffer.writeln();
    buffer.writeln(
      '# ${subject.code} — Materials',
    );
    buffer.writeln();

    if (syllabus == null ||
        syllabus.materials.isEmpty) {
      buffer.writeln(
        'No learning materials were retrieved from FLM.',
      );
...
''';
  ...
  Future<String> _uniqueDirectoryPath(
    String desiredPath,
  ) async {
    if (!await Directory(
      desiredPath,
  ...
  Future<void> _writeGeneratedFile(
    File file,
    String content,
  ) async {
